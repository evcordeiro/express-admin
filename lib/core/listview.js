    ...
    args.db.client.query(args.query, function (err, rows) {
        ...
        for (var i=0; i < rows.length; i++) {
            ...
            for (var j=1; j < columns.length; j++) {
                ...
                if (columns[j].listview.showAsImage){
                  value = {
                    value: value,
                    lambda: function(){ return function(value) {
                      return '<img class="listview_showasimage" src="' + value.trim() + '"/>';
                  }}};
                }
                values.push(value);
            }
...
function format (column, value) {
    ...
    switch(true)
    {
      case (/^(date|datetime|timestamp).*/i.test(column.type)):
      ...
      case (column.control.radio):
        ...
        break;
      case column.listview.showAsImage:
        return column.listview.imagePrefix + value;
      default: 
        return value;
        break;
    }
